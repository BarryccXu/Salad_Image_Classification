...
def compute_accuracy(v_xs, v_ys):
    global prediction
    y_pre = sess.run(prediction, feed_dict={Xs: v_xs, keep_prob: 0.7})
    correct_prediction = tf.equal(tf.argmax(y_pre,1), tf.argmax(v_ys,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    result = sess.run(accuracy, feed_dict={Xs: v_xs, ys: v_ys, keep_prob: 0.7})
    return result
...
Xs = tf.placeholder(tf.float32, [batch_size, 128*128*3])
...
with tf.device('/gpu:0'):
    ...
    for epoc in range(100):
        ...
        for i in range(np.int32(numOfTrain*n_classes/batch_size)):
            ...
            result, _ = sess.run([cross_entropy, train_step], 
                              {Xs: X_train_shuffled[i_start:i_end,:], 
                               ys: y_train_oneHot[i_start:i_end,:], keep_prob: 0.6})
#            result = compute_accuracy(X_train_shuffled[i_start:i_end,:], y_train_oneHot[i_start:i_end,:])
            #costs.append(c)
            #print('batch:', result)
            y_pre = sess.run(prediction, feed_dict={Xs: X_train_shuffled[i_start:i_end,:], 
                               ys: y_train_oneHot[i_start:i_end,:], keep_prob: 0.7})
            correct_prediction = tf.equal(tf.argmax(y_pre,1), tf.argmax(y_train_oneHot[i_start:i_end,:],1))
            tmp = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
            accuracy = sess.run(tmp, feed_dict={Xs: X_train_shuffled[i_start:i_end,:], 
                               ys: y_train_oneHot[i_start:i_end,:], keep_prob: 0.7})
            accuracys.append(accuracy)
            results.append(result)
        
        print(epoc)
        print('loss:', np.mean(results))
        results_epoc.append(np.mean(results))
        print('accuracy:', np.mean(accuracys))
        accuracy_epoc.append(np.mean(accuracys))
            #print('loss:', c)
...
plot_conv_layer(W_conv3, X_train_shuffled[0:10,:])

plt.show()
plt.plot(results_epoc)
plt.show()        
